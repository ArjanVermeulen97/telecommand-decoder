  ...
  if (state == 0) {
    ...
    if (Serial.available() >= 8) {
      ...
      if (recv_bytes >= 8) {
        ...
        for (int i=0; i<7; i++) {
          frame_buffer[i+7*cltu_count] = recv_buffer[i];
        }
        cltu_count = cltu_count+1;
        ...
        if (received_header == false) {
          ...
          AMOUNT_CLTU = ceil((FRAME_LENGTH + 40 + 16)/(double)56);
        ...
        if (cltu_count == AMOUNT_CLTU) {
         Serial.println("<Received all CLTUs>"); 
         for (int i=0; i<21; i++) {
              Serial.print(">");
              Serial.print(i);
              Serial.print(": ");
              printBits(frame_buffer[i]);
              Serial.print("\n");
            } 
         unsigned int remainder = crcframe(frame_buffer);
         Serial.print(">Remainder: ");
         Serial.println(remainder, BIN);
         if (remainder == 0 ){
...
unsigned int crcframe(byte cltu_blocks[21]) {
  byte bin_array[168];
  byte mask = B10000000;
  ...
  for (int i=0; i<152; i++) {
    if(bin_array[i] == 1) {
      // XOR 0b1_0001_0000_0010_0001
      // x^16 + x^12 + x^5 + x^0
      bin_array[i] = 1 - bin_array[i];
      bin_array[i+4] = 1 - bin_array[i+4];
      bin_array[i+11] = 1 - bin_array[i+11];
      bin_array[i+16] = 1 - bin_array[i+16];
    }
  }

  //{0, 0, 0, 0, 0, 0, 0, ..........., 1, 0, 1, 1, 1, 0, 0, 0} -> 10111000
  for (int i=152; i<168; i++) {
    result = result + (bin_array[i] << (167-i));
  }
